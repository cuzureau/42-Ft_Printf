...
/*   Updated: 2018/10/19 13:00:07 by cuzureau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_add_number(t_field *f, t_main *m)
{
	...
	ft_get_number(&n, m, f);
	ft_prefix(&n, f);
	if (f->prec < (ft_strlen(n.start) + ft_strlen(n.prefix)))
		n.len = f->widt - (ft_strlen(n.prefix) + ft_strlen(n.start));
	else
		n.len = f->widt - f->prec - ft_strlen(n.prefix);
	if (!(f->flag & MINUS))
	{
		if (!(f->flag & ZERO) || (f->flag & ZERO && f->prec > 0))
			ft_fill(m, ' ', n.len);
		ft_add(n.prefix, ft_strlen(n.prefix), m, f);
		if (f->prec > 0)
			ft_fill(m, '0', f->prec - ft_strlen(n.start) - \
	...
	{
		ft_add(n.prefix, ft_strlen(n.prefix), m, f);
		f->prec > 0 ? ft_fill(m, '0', (f->prec - ft_strlen(n.start))) : 0;
	}
	ft_add(n.start, ft_strlen(n.start), m, f);
